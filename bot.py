...
def parse_wordle_result(text):
    ...
    is_wordle_context = (
        ...
        bool(re.search(r'la palabra del d[ií]a', text_lower)) or
        bool(re.search(r'wordle', text_lower)) or
        bool(re.search(r'lapalabradeldia', text_lower))
    )

    if not is_wordle_context:
        return None
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Soportar texto puro Y caption (imagen + texto como lapalabradeldia)
...
async def reminder_20h(context: ContextTypes.DEFAULT_TYPE):
    """Reminder a las 20:00 mencionando a los que faltan."""
    scores = load_scores()
    for chat_id, chat_data in scores.items():
        ...
        missing_ids = [uid for uid in players if uid not in pending]

        if not missing_ids:
            continue  # Todos ya han compartido
        ...
        msg = f"⏰ ¡Recordatorio! Faltan por compartir el Wordle de hoy:\n{', '.join(mentions)}\n\n¡Quedan pocas horas! 🟩🟨⬛"
        ...
        try:
            ...
            logging.error(f"Error enviando reminder al chat {chat_id}: {e}")
...
def main():
    ...
    # Jobs diarios
    app.job_queue.run_daily(reminder_20h, time=dtime(20, 0, 0))
    ...
    # Escuchar tanto mensajes de texto como mensajes con foto (caption)
